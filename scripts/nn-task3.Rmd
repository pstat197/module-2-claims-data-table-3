...
```{r}
# 3. Logistic PCR baseline (for comparison)
...
# Fit a neural network (one hidden layer with `size` units)
# You can experiment with `size`, `decay`, and `maxit`.
set.seed(197)
nn_fit <- nnet(
  bclass ~ .,
  data   = df_train_nn,
...
cat("\nNeural net model and Task 3 accuracy saved in '../results/'.\n")
```
