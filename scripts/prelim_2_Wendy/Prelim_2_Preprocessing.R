...
# bigram pipeline for task 2
# This mirrors nlp_fn but uses 2-word ngrams instead of single words
nlp_bigram_fn <- function(parse_data.out){
  out <- parse_data.out %>% 
    unnest_tokens(
      output = bigram,
      input  = text_clean,
    ...
    # optional: drop bigrams with digits or very short tokens
